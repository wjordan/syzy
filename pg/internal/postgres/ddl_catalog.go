...
// OpDropUniqueKey. A CHECK / FOREIGN KEY / EXCLUDE constraint changes no column,
// no name, and no unique key, so it produces no op and no error — it stays local
// to this node. For CHECK and EXCLUDE that would quarantine every peer write the
// constraint rejects, so the admission gate refuses them before they can commit
// (syzy_ddl_restricting_cons); a foreign key is not enforced on applied rows at
// all, so it is admitted and simply does not travel.
func buildAlterTableOps(ctx context.Context, conn *pgx.Conn, cat *catalog, in ddlIntent) ([]crdt.CatalogOp, error) {
	...
	// A trigger left ENABLE ALWAYS / ENABLE REPLICA also fires on the writes
	// apply makes under session_replication_role = replica, so a peer's row lands
	// here plus whatever the trigger writes on top of it — a local-only mutation
	// no peer ever sees. The admission gate refuses the ALTER that enables one;
	// this is the floor for a node whose DDL support arrived after it was set.
	var alwaysTriggers []string
	if err := conn.QueryRow(ctx, `
		SELECT COALESCE(array_agg(tgname ORDER BY tgname), '{}')
		FROM pg_trigger WHERE tgrelid = $1 AND NOT tgisinternal AND tgenabled IN ('A','R')`,
		in.objid).Scan(&alwaysTriggers); err != nil {
		return nil, fmt.Errorf("postgres: ALTER TABLE %s: trigger modes: %w", ti.name, err)
	}
	if len(alwaysTriggers) > 0 {
		return nil, unsupportedDDLf("postgres: ALTER TABLE %s: trigger(s) %s are ENABLE ALWAYS/REPLICA; they would also fire on applied peer writes and diverge this node", ti.name, strings.Join(alwaysTriggers, ", "))
	}
	...
	for _, pc := range pgcols {
		...
		if !existed {
			...
			// Same hazard, non-sequence form: a non-immutable default (now(),
			// gen_random_uuid(), random()) is evaluated per node when Postgres
			// backfills the existing rows.
			if pc.volatileDefault {
				return nil, unsupportedDDLf("postgres: ALTER TABLE %s: ADD COLUMN %q DEFAULT %s is not immutable; Postgres backfills existing rows by evaluating it on this node, so every node would store different values — add the column without a default, then SET DEFAULT and backfill with an UPDATE", ti.name, pc.name, pc.def)
			}
			cid := catalogpkg.AllocColumnID()
		...
		if changed {
			if pc.typeName != ci.typeName && hasUsingClause(in.auditQuery) {
				// USING rewrote THIS node's stored values, and the op carries only
				// the target type — a follower's plain ALTER COLUMN … TYPE keeps its
				// own rows and the two silently diverge. Nothing in pg_catalog
				// records a USING clause, so this is the one place the audit text is
				// consulted: as an admission heuristic that errs toward rejection,
				// never to build an op.
				return nil, unsupportedDDLf("postgres: ALTER TABLE %s: ALTER COLUMN %q TYPE … USING rewrites this node's rows only and cannot replicate; widen the column without USING, then UPDATE the values as ordinary DML", ti.name, pc.name)
			}
...
type pgColumn struct {
	...
	// volatileDefault is true when the default expression names a non-immutable
	// function, so ADD COLUMN would backfill existing rows with values only this
	// node computes.
...
func introspectColumns(ctx context.Context, conn *pgx.Conn, oid uint32) ([]pgColumn, error) {
	rows, err := conn.Query(ctx, `
		SELECT a.attnum, a.attname, format_type(a.atttypid, a.atttypmod),
		       ...
		       -- Non-immutable default: every function the stored expression names
		       -- (:funcid / :opfuncid in the node tree) is checked for volatility.
		       -- Only matters on ADD COLUMN, which backfills existing rows by
		       -- evaluating it on THIS node.
		       EXISTS (
		         SELECT 1 FROM regexp_matches(d.adbin::text, ':(?:funcid|opfuncid) ([0-9]+)', 'g') m
		         JOIN pg_proc pr ON pr.oid = m[1]::oid WHERE pr.provolatile <> 'i'
		       ) AS volatile_default
		FROM pg_attribute a
	...
	for rows.Next() {
		...
		var ownsSeq bool
		var ident string
		if err := rows.Scan(&c.attnum, &c.name, &c.typeName, &c.notNull, &c.def, &c.generated, &c.pkpos, &ownsSeq, &ident, &c.counter, &c.volatileDefault); err != nil {
			return nil, err
		}
		if c.counter {
...
func classifyColumnChange(tname string, ci *colInfo, pc pgColumn) (bool, error) {
	...
	case pc.typeName != ci.typeName && autoIncrementShape(ci, pc):
		// An auto-increment column's type does not travel as a type: catalogColumn
		// ships it as the CREATE-shaped "bigserial" / "… GENERATED … AS IDENTITY",
		// neither of which is spellable in ALTER COLUMN … TYPE. Emitting the op
		// would leave every follower failing the statement forever.
		return false, unsupportedDDLf("postgres: ALTER TABLE %s: changing the type of auto-increment column %q (serial/identity) is not supported; its type carries the sequence shape a follower cannot re-declare", tname, pc.name)
...
// hasUsingClause reports whether q contains a bare USING keyword. Deliberately
// crude — it only ever adds a rejection, and the alternative (a type change
// whose USING silently rewrote the origin's rows) is undetectable divergence.
func hasUsingClause(q string) bool {
...
// autoIncrementShape reports whether a column mints its own values on either
// side of an ALTER — an IDENTITY column, or a serial (owned sequence + nextval
// default). Checked on BOTH the cached and the live shape so the answer does
// not depend on which direction the ALTER went.
func autoIncrementShape(ci *colInfo, pc pgColumn) bool {
	return ci.identity != 0 || pc.identity != 0 ||
		strings.HasPrefix(ci.def, "nextval(") || pc.serialType != ""
}
